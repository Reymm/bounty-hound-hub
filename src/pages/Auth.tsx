    ...
    try {
      ...
      if (error) {
        if (error.message.includes('Invalid login credentials')) {
          ...
          // We show a helpful message suggesting they might have signed up with Google
          setError('google_or_invalid');
        ...
        <Card>
          ...
          <CardContent>
            <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
              ...
              {error && error !== 'email_not_confirmed' && error !== 'google_or_invalid' && (
                <Alert variant="destructive" className="mt-4">
                  <AlertDescription>{error}</AlertDescription>
                </Alert>
              )}

              {error === 'google_or_invalid' && (
                <Alert variant="destructive" className="mt-4">
                  <AlertDescription className="space-y-3">
                    <p className="font-semibold">Invalid email or password</p>
                    <p className="text-sm">
                      Please check your credentials. If you signed up with Google, use the "Continue with Google" button above instead.
                    </p>
