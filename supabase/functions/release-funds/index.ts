...
serve(async (req) => {
  ...
  try {
    ...
    return new Response(JSON.stringify({ 
      error: isAuthError ? 'Authorization failed' : isNotFound ? 'Resource not found' : 'Fund release failed',
      code: isAuthError ? 'AUTH_ERROR' : isNotFound ? 'NOT_FOUND' : 'RELEASE_ERROR'
    }), {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
