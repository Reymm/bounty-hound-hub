-- Create enum for user report types (only if it doesn't exist)
DO $$ BEGIN
...
-- Create user_reports table (only if it doesn't exist)
...
DROP POLICY IF EXISTS "Support admins can view all reports" ON public.user_reports;
...
DROP POLICY IF EXISTS "Support admins can update reports" ON public.user_reports;
