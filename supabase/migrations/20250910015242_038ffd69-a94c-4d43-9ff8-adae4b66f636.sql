...
-- 3. Drop existing storage policies and create strict ones for bounty-images bucket
DROP POLICY IF EXISTS "Users can upload bounty images to their folder" ON storage.objects;
DROP POLICY IF EXISTS "Users can view bounty images" ON storage.objects;
DROP POLICY IF EXISTS "Users can update their own bounty images" ON storage.objects;
DROP POLICY IF EXISTS "Users can delete their own bounty images" ON storage.objects;
...
-- 4. Drop existing and create strict storage policies for submission-files bucket
DROP POLICY IF EXISTS "Users can upload submission files to their folder" ON storage.objects;
DROP POLICY IF EXISTS "Bounty participants can view submission files" ON storage.objects;
...
-- 5. Drop existing and create strict storage policies for avatars bucket
...
-- 6. Drop existing and create strict storage policies for documents bucket
