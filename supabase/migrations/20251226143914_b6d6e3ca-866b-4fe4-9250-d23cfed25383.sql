-- Drop and recreate views with SECURITY INVOKER to respect caller's RLS
DROP VIEW IF EXISTS public.bounties_secure;
DROP VIEW IF EXISTS public.profiles_secure;

-- Recreate bounties_secure with SECURITY INVOKER (explicit)
...
-- Recreate profiles_secure with SECURITY INVOKER (explicit)
...
-- Add comments
COMMENT ON VIEW public.bounties_secure IS 'Secure view that hides shipping_details from unauthorized users';
COMMENT ON VIEW public.profiles_secure IS 'Secure view that hides payment info from other users';